   ...
   R=st.executeQuery("select * from patient  where patientid ='"+PatientId+"';");
    while(R.next())
    {
       ...
       // Patientdetails.put("middleName",R.getString("middlename"));
        ...
        Patientdetails.put("bloodGroup",R.getString("bloodbgoup"));
...
int a=0;        
  ...
  // succ=st.executeUpdate("insert into appointments  (doctorid,patientid,day,timeslotid,confirmation) values('"+Doctorid+"','"+Patientid+"','"+apntdate+"',"+Timeslotid+");");
     ...
     public int editpatient(String patid,String phone,String email,String bloodgroup,String username,String password)
   {
       int succ=0;
       try{
           succ=st.executeUpdate("update patient set phoneno='"+phone+"',email='"+email+"',bloodbgoup='"+bloodgroup+"',username='"+username+"' where patientid='"+patid+"';");
       }
       catch(Exception E)
       {
           String msg=E.getMessage();
...
return D;
   ...
   {
    try{
   int a=st.executeUpdate("delete from appointments where appointmentid ='"+apptid+"';");
   }
   catch(Exception E)
           {
           return false;
   ...
   return(true);
    ...
    ResultSet getnursedetailds() {
        throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    }

    ResultSet getadmindetailds() {
        throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    }

    ResultSet getAppoinmentsforpatients() {
         try{
   R=st.executeQuery("select a.appointmentid, e.fname,a.day,t.value,a.confirmation from employeee e,timeslot t,appointments a where a.doctorid =e.employee_id and a.timeslotid =t.id ;");
   }
   catch(Exception E)
           {
           return null;
